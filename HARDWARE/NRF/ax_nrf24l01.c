...
//IO操作宏定义
#define NRF24L01_CE_H()   GPIO_SetBits(GPIOB, GPIO_Pin_12)        
#define NRF24L01_CE_L()   GPIO_ResetBits(GPIOB, GPIO_Pin_12) 
#define NRF24L01_CSN_H()  GPIO_SetBits(GPIOB, GPIO_Pin_8)        
#define NRF24L01_CSN_L()  GPIO_ResetBits(GPIOB, GPIO_Pin_8)
#define NRF24L01_IRQ     (GPIO_ReadInputDataBit(GPIOB, GPIO_Pin_9))
...
//文件内部函数声明
static uint8_t SPI2_Send_Byte(uint8_t dat);
...
uint8_t AX_NRF24L01_Init(void)
{
	...
	SPI_InitStructure.SPI_BaudRatePrescaler = SPI_BaudRatePrescaler_8;//10.5M;
	...
	//开启IRQ中断
	GPIO_EXTILineConfig(GPIO_PortSourceGPIOB,GPIO_PinSource9);//开启GPIO管脚的中断线路
...
static uint8_t NRF24L01_ReadReg(uint8_t reg)
{
	...
	SPI2_Send_Byte(reg);            // Select register to read from..
	reg_val = SPI2_Send_Byte(0);    // ..then read registervalue
...
static uint8_t NRF24L01_WriteReg(uint8_t reg, uint8_t dat)
{
	...
	status = SPI2_Send_Byte(reg);   // select register
	SPI2_Send_Byte(dat);          // ..and write value to it..
...
static uint8_t NRF24L01_ReadBuf(uint8_t reg, uint8_t *pBuf, uint8_t len)
{
	...
	status = SPI2_Send_Byte(reg);       		// Select register to write to and read status uint8_t
